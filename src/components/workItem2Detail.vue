  ...
  <div>
    ...
    <v-card class="mx-auto blue lighten-5" max-width="550" align="left" outlined>
      ...
      <v-card-actions>
        ...
        <v-spacer></v-spacer>
        <v-btn text color="indigo" @click="ProgressAdd(newProgress,propData.id,true,-1)">進度填報</v-btn>
      </v-card-actions>
    </v-card>
...
<script>
import workItem3Edit from "./workItem3Edit";
...
export default {
  ...
  methods: {
    ...
    workItemDetailEdit() {
      ...
      //淺拷貝，progress還是有被複製
      // this.editWorkItemDetailData = Object.assign({}, this.propData);
      // 深層複製一份給子元件編輯用，
      this.editWorkItemDetailData = JSON.parse(JSON.stringify(this.propData));
    },
    //編輯後，子元件返回
    getChildData_ItemDetailEdit(childData) {
      this.detailEditDialog = false;
      if (!childData) {
        //按取消 this.propData沒變，return
        return false;
      }
      //被編輯處理過的資料，更動傳進來的參數this.proData
      this.propData.title = this.propData.t_title; //還原title
      this.propData.depart = childData.depart;
      this.propData.endDate = childData.endDate;
      this.propData.startDate = childData.startDate;
      ...
      if (
        ...
        days = moment(this.propData.endDate).diff(moment(), "day");
        this.propData.remaindays = `<span class="red--text">${days}天</span>`;
      } else {
        this.propData.remaindays = "";
      }

      // console.log("propData傳入但已更動", this.propData,"return",childData);
      //傳回 propData 資料給父層處理 資料庫存檔 及 更改螢幕陣列物件
